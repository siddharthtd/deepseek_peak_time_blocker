...
while read -r clock want; do
  if AI_WINDOW_TEST_NOW_UTC="${clock}" "${GUARD}" >/dev/null 2>&1; then got=allow; else got=deny; fi
  if [[ "${got}" == "${want}" ]]; then
    ok "${clock} -> ${got}"
  else
...
peak="$(printf '%s' '{"hook_event_name":"PreToolUse","tool_name":"read_file"}' |
  AI_WINDOW_TEST_NOW_UTC=02:00 "${GUARD}" --hook)"
case "${peak}" in
...
open="$(printf '%s' '{"hook_event_name":"PreToolUse","tool_name":"read_file"}' |
  AI_WINDOW_TEST_NOW_UTC=11:00 "${GUARD}" --hook)"
case "${open}" in
  '{}' | '{"hookSpecificOutput"'*)
...
if AI_WINDOW_TEST_NOW_UTC=02:00 "${GUARD}" override --minutes 30 --reason verify </dev/null >/dev/null 2>&1; then
...
armed="$(printf '%s' "{\"hook_event_name\":\"PreToolUse\",\"tool_name\":\"run_in_terminal\",\"tool_input\":{\"command\":\"${GUARD} over""ride --minutes 60\"}}" |
  AI_WINDOW_TEST_NOW_UTC=12:00 "${GUARD}" --hook)"
case "${armed}" in
...
elif [[ -s "${STATE_DIR}/watch.log" ]]; then
  ok "watcher has run (log: ${STATE_DIR}/watch.log)"
else
